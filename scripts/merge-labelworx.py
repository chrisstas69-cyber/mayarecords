...
def main() -> None:
    ...
    for cat, rel in by_cat.items():
        has_master = cat in masters
        rel["digital_price_cents"] = price_for(len(masters[cat])) if has_master else None
    ...
    for_sale = sum(1 for r in merged if r.get("digital_price_cents"))
    print(f"{len(merged)} releases · {len(lw)} from Labelworx · {with_preview} with previews · {for_sale} for sale (full masters on hand)")
